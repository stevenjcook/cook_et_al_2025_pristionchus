...
class ParseTrakEM2(object):
    ...
    def __setstate__(self, state):
        ...
        parser = etree.XMLParser(remove_blank_text=True, huge_tree=True)
        self.xml = etree.parse(self.trakem2, parser)
    ...
    def get_layers(self):
        ...
        title = self.xml.xpath("//t2_layer/@z")
    ...
    def get_connectors(self):
        ...
        """
        connectors = self.xml.xpath("//t2_connector/@title")
        trans = self.xml.xpath("//t2_connector/@transform")
        oid = self.xml.xpath("//t2_connector/@oid")
        self.connectors = {}
        for i in range(len(connectors)):
            ...
            temp[1] -= self.dy
            temp = tuple(temp)
            C = Connector(connectors[i])
            C.oid = oid[i]
            C.transform = temp
            self.connectors[C.name] = C
        if 'connectors' in self.connectors.keys():
    ...
    def get_area_lists(self):
        ...
        (key=cell name, val=AreaList(object))
        """
        area_lists = self.xml.xpath("//t2_areatree/@title")
        trans = self.xml.xpath("//t2_areatree/@transform")
        self.area_lists = {}
    ...
    def get_calibration(self):
        ...
        trans = self.xml.xpath("//t2_layer_set/@transform")[0]	
        trans = trans.replace(')','')
        trans = trans.split(',')
    ...
    def get_boundaries_in_layer(self,layer,scale_bounding_box=1,
        ...
        boundary = {}
        for n in area_lists:
            path = ("//t2_areatree[@title='%s']"
                    "//t2_node[@lid='%s']"
                    "/t2_area/t2_path/@d" %(n,layer.oid))
    ...
    def get_boundary(self, l, graph=True):
        L = sorted(self.layers.keys())
        layer = self.layers[L[l-1]]
        boundary = {}
        for n in list(self.area_lists.keys()):
            path = ("//t2_areatree[@title='%s']"
                    "//t2_node[@lid='%s']"
                    "/t2_area/t2_path/@d" %(n,layer.oid))
            path = self.xml.xpath(path)
            ...
            for p in path:
                p = self.area_lists[n].path_transform(p)
                #if self.compute_area(p) > self.area_thresh:
                p = self.fill_boundary_gaps(p)
                temp += p
            if temp:
                boundary[n] = temp 
         
        if graph:
            for nuc in list(boundary.keys()):
                X = []
                Y = []
                for x in boundary[nuc]:
                    X.append(x[0])
                    Y.append(x[1])
                plt.scatter(X, Y, label=nuc)
            plt.legend()
            plt.title(L[l-1])
            plt.show() 
        ...
        neuron_order = sorted(boundary.keys())
        N = len(neuron_order)
        neuron = dict(zip(neuron_order,range(N)))
        A = np.zeros([N,N],dtype=np.int)
        
        for nuc in boundary.keys():
            for p in set(boundary[nuc]):
                if p not in pixels:
                    pixels[p] = []
                if nuc not in pixels[p]:
                    pixels[p].append(nuc) 
          
        nodes = pixels.copy().keys()
        for p in nodes:
            if len(pixels[p]) > 1:
                comb = list(itertools.combinations(pixels[p],2))
                for (n1,n2) in comb:
                    A[neuron[n1],neuron[n2]] += 1
                    A[neuron[n2],neuron[n1]] += 1
                del pixels[p]
            else:
                pixels[p] = pixels[p][0]
        ...
        for i in range(len(idx[0])):
            ...
            q = idx[1][i]
            if q > p:
                data.append([neuron_order[p],
                             neuron_order[q],
                             L[l-1],A[p,q]])      
                
        size_dict = {}
        for nuc in boundary.keys():
            size_dict[nuc] = [self.compute_area(boundary[nuc]), self.compute_centroid(boundary[nuc])]
                
        return size_dict, data	
		
    def compute_area(self, B):
        area = 0	
        for i in range(len(B)):
            area += (B[i][0]*B[i-1][1] - B[i-1][0]*B[i][1]) 		
        return 0.5*abs(area)
    
    def compute_centroid(self, B):
        x = [p[0] for p in B]
        y = [p[1] for p in B]
        centroid = (sum(x)/len(B), sum(y)/len(B))
        return centroid
    
    def fill_boundary_gaps(self, B):
        zB = list(zip(B[:-1],B[1:]))
        zB.append((B[-1],B[0]))
        cnts = []
        for (c1,c2) in zB:
...
class Connector(object):
    """
    Class used to hold connector information
